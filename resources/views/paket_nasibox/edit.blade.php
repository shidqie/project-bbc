        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-[8px]">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('paket-nasibox.update', $paketNasibox->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="nama" id="nama" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" value="{{ $paketNasibox->nama }}" required>
                                </div>

                                <div>
                                    <label for="deskripsi" class="block font-medium text-sm text-gray-700">Deskripsi</label>
                                    <textarea name="deskripsi" id="deskripsi" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm">{{ $paketNasibox->deskripsi }}</textarea>
                                </div>

                                <div>
                                    <label for="harga" class="block font-medium text-sm text-gray-700">Harga (Rp)</label>
                                    <input type="number" name="harga" id="harga" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" value="{{ $paketNasibox->harga }}" required>
                                </div>
                                ...
                                <div>
                                    ...
                                    @if($paketNasibox->gambar)
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $paketNasibox->gambar) }}" class="w-32 h-32 object-cover rounded-[4px]">
                                        </div>
                                    @endif
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div id="bom-container" class="space-y-3">
                                        @forelse($paketNasibox->detail as $detail)
                        ...
                        <div class="flex items-center gap-4 pt-4 border-t border-gray-100">
                            ...
                            <a href="{{ route('paket-nasibox.index') }}" class="text-gray-500 hover:text-gray-700 font-medium">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
