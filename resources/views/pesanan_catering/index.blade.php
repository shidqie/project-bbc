<x-app-layout>
    <x-slot name="header">Pesanan Catering</x-slot>

    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-0">Pesanan Catering</h4>
            <small class="text-muted">Kelola semua pesanan catering</small>
        </div>
        @if(in_array(auth()->user()->role, ['pemilik','manager']))
        <a href="{{ route('pesanan-catering.create') }}" class="btn btn-primary"><i class="bi bi-plus me-1"></i>Tambah Pesanan</a>
        @endif
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @endphp
                        <tr>
                            <td class="ps-4 text-muted small">CA-{{ str_pad($p->id,4,'0',STR_PAD_LEFT) }}</td>
                            ...
                            <td>
                                <span class="badge bg-{{ $color }}" style="font-size:11px;">
                                    {{ str_replace('_',' ', ucfirst($p->status)) }}
                                </span>
                            </td>
                            <td class="text-end fw-bold">Rp {{ number_format($p->total_harga,0,',','.') }}</td>
                            <td class="text-end pe-4">
                                <a href="{{ route('pesanan-catering.show',$p->id) }}" class="btn btn-sm btn-outline-primary">Detail</a>
                        ...
                        @empty
                        <tr><td colspan="8" class="text-center text-muted py-5">Belum ada pesanan catering.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    function prepModal(btn){
        document.getElementById('statusForm').action='/pesanan-catering/'+btn.dataset.id+'/status';
