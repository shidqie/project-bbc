<x-public-layout>
    ...
    <section class="py-5 mt-2">
        <div class="container py-3" style="max-width:820px;">
            <a href="{{ route('public.nasibox') }}" class="btn btn-outline-secondary btn-sm rounded-pill mb-4">
                <i class="bi bi-arrow-left me-1"></i>Kembali
            </a>
            <div class="card border-0 shadow-sm" data-aos="fade-up">
                <div class="card-body p-4 p-md-5">
                    <span class="badge-kategori mb-3 d-inline-block">Paket Nasi Box</span>
                    <h1 class="fw-black mb-2" style="font-size:2rem;">{{ $paket->nama }}</h1>
                    <p class="text-muted mb-4" style="line-height:1.8;">{{ $paket->deskripsi ?? 'Nasi box spesial dengan pilihan isi terbaik.' }}</p>
                    <div class="bg-light rounded-3 p-3 d-inline-block mb-4">
                        ...
                        <div class="text-muted small">per box</div>
                    </div>
                    @if($paket->detail && $paket->detail->count())
                    <h5 class="fw-bold mb-3">Isi Paket</h5>
                    ...
                    @endif
                    <a href="{{ route('public.order.nasibox',['paket_id'=>$paket->id]) }}"
                       class="btn btn-bbc btn-lg rounded-pill px-5 fw-bold">
                        <i class="bi bi-box-seam me-2"></i>Pesan Sekarang
                    </a>
                </div>
            </div>
        </div>
