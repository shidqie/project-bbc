<x-public-layout>
    <x-slot name="title">Pesan Nasi Box</x-slot>
    <section class="py-5 mt-2">
        <div class="container py-3" style="max-width:720px;">
            <div class="text-center mb-5" data-aos="fade-up">
                <span class="badge-kategori mb-3 d-inline-block">Form Pemesanan</span>
                <h1 class="section-title">Pesan Nasi Box</h1>
                <p class="section-sub">Isi data berikut, pesanan akan segera diproses</p>
            </div>
            ...
            <div class="card border-0 shadow-sm" data-aos="fade-up">
                <div class="card-body p-4 p-md-5">
                    <form action="{{ route('public.order.nasibox.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold small">Paket Nasi Box <span class="text-danger">*</span></label>
                                <select name="paket_nasibox_id"
                                    class="form-select rounded-3{{ $errors->has('paket_nasibox_id') ? ' is-invalid' : '' }}"
                                    id="paket-select" onchange="updateHarga()">
                                    <option value="">— Pilih Paket —</option>
                                    @foreach($pakets as $p)
                                    <option value="{{ $p->id }}"
                                        data-harga="{{ $p->harga }}"
                                        {{ (old('paket_nasibox_id') == $p->id || $selectedId == $p->id) ? 'selected' : '' }}>
                                        {{ $p->nama }} — Rp {{ number_format($p->harga,0,',','.') }}/box
                                    </option>
                                    @endforeach
                                </select>
                                <div class="invalid-feedback">{{ $errors->first('paket_nasibox_id') }}</div>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <label class="form-label fw-semibold small">Tanggal Kirim <span class="text-danger">*</span></label>
                                <input type="date" name="tanggal_kirim" value="{{ old('tanggal_kirim') }}"
                                    min="{{ date('Y-m-d') }}"
                                    class="form-control rounded-3{{ $errors->has('tanggal_kirim') ? ' is-invalid' : '' }}">
                                <div class="invalid-feedback">{{ $errors->first('tanggal_kirim') }}</div>
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-semibold small">Catatan</label>
                                <textarea name="catatan" rows="2" class="form-control rounded-3"
                                    placeholder="Permintaan khusus, alergi makanan...">{{ old('catatan') }}</textarea>
                            </div>
                        </div>
    ...
    function updateHarga(){
        ...
        if(harga&&qty){ document.getElementById('harga-total').textContent='Rp '+(harga*qty).toLocaleString('id-ID'); prev.classList.remove('d-none'); }
