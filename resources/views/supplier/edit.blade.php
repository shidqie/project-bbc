                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('supplier.update', $supplier->id) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama" id="nama" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" value="{{ $supplier->nama }}" required>
                        </div>

                        <div>
                            <label for="no_hp" class="block font-medium text-sm text-gray-700">Nomor Handphone</label>
                            <input type="text" name="no_hp" id="no_hp" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" value="{{ $supplier->no_hp }}" required>
                        </div>

                        <div>
                            <label for="alamat" class="block font-medium text-sm text-gray-700">Alamat</label>
                            <textarea name="alamat" id="alamat" rows="3" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" required>{{ $supplier->alamat }}</textarea>
                        </div>
