        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-[8px]">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('paket-catering.update', $paketCatering->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="nama" id="nama" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" value="{{ $paketCatering->nama }}" required>
                                </div>

                                <div>
                                    <label for="deskripsi" class="block font-medium text-sm text-gray-700">Deskripsi</label>
                                    <textarea name="deskripsi" id="deskripsi" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm">{{ $paketCatering->deskripsi }}</textarea>
                                </div>

                                <div>
                                    <label for="harga" class="block font-medium text-sm text-gray-700">Harga (Rp)</label>
                                    <input type="number" name="harga" id="harga" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" value="{{ $paketCatering->harga }}" required>
                                </div>

                                <div>
                                    <label for="minimum_order" class="block font-medium text-sm text-gray-700">Minimum Order (Porsi)</label>
                                    <input type="number" name="minimum_order" id="minimum_order" class="mt-1 block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm" value="{{ $paketCatering->minimum_order }}" required>
                                </div>
                                ...
                                <div>
                                    ...
                                    @if($paketCatering->gambar)
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $paketCatering->gambar) }}" class="w-32 h-32 object-cover rounded-[4px]">
                                        </div>
                                    @endif
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div id="bom-container" class="space-y-3">
                                        @forelse($paketCatering->detail as $detail)
                                        <div class="flex gap-2 bom-row">
                                            <select name="menu_id[]" class="block w-full border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm text-sm" required>
                                                ...
                                                @foreach($menus as $menu)
                                                    <option value="{{ $menu->id }}" {{ $detail->menu_id == $menu->id ? 'selected' : '' }}>{{ $menu->nama }}</option>
                                                @endforeach
                                            </select>
                                            <input type="number" name="jumlah_porsi[]" value="{{ $detail->jumlah_porsi }}" class="block w-24 border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm text-sm" min="1" required>
                                        ...
                                        <div class="flex gap-2 bom-row">
                                            ...
                                            <input type="number" name="jumlah_porsi[]" class="block w-24 border-gray-300 focus:border-[#3B82F6] focus:ring-[#3B82F6] rounded-[4px] shadow-sm text-sm" min="1" value="1" required>
                                            <button type="button" class="px-2 py-1 bg-red-100 text-red-600 rounded-[4px] hover:bg-red-200" onclick="this.parentElement.remove()">X</button>
                                        </div>
                                        @endforelse
                                    </div>
